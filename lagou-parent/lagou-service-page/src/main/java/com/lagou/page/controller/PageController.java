...
public class PageController {
    ...
    /**
     * 模拟服务超时，熔断处理
     * 针对熔断处理，Hystrix默认维护一个线程池，默认大小10
     * @return
     */
    @HystrixCommand(
        // 只要是在@HystrixCommand中定义了threadPoolKey，就意味着开启了舱壁模式（线程隔离），该方法就会自己维护一个线程池
        threadPoolKey = "getProductServicePort2", // 默认所有的请求共同维护一个线程池，实际开发，每个方法维护一个线程池
                //  给一个属性对应的都是一个threadPoolProperty
        threadPoolProperties = {
            @HystrixProperty(name = "coreSize", value = "1"), //并发线程数
            @HystrixProperty(name = "maxQueueSize", value = "20") //等待队列，默认线程队列值是-1，默认不开启
        },
        //超时时间的设置
        commandProperties = {
    ...
    public String getProductServicePort2() {
        return productFeign.getPort();
    }
    ...
    public String getProductServicePort3() {
        return productFeign.getPort();
    }
